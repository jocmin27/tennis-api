...
class PlayerServiceImplTest {
  ...
  @Test
  void savePlayer() {
    var player = createPlayer("Jordan", "male");
    when(playerRepository.save(any(Player.class))).thenReturn(Mono.just(player));

    Mono<Player> playerMono = playerService.savePlayer(createPlayerRequest("Jordan", "male"));
    StepVerifier.create(playerMono).assertNext(resultResponse -> {
        assertEquals("Jordan", resultResponse.getName());
  ...
  void getListPlayer() {
    ...
    StepVerifier.create(playerMono)
      ...
      .assertNext(resultResponse -> {
        assertEquals("Junior", resultResponse.getName());
        assertEquals("male", resultResponse.getGender());
      })
      .verifyComplete();
  }
